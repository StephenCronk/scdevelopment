...
`,de={maxSteps:128,balls:7,aoTaps:5,maxDpr:2},he={maxSteps:72,balls:5,aoTaps:3,maxDpr:1.5},Z=.55,N=1,J=12.4;function fe(n){const c=parseInt(n.replace("#",""),16),i=h=>{const t=h/255;return t<=.04045?t/12.92:Math.pow((t+.055)/1.055,2.4)};return[i(c>>16&255),i(c>>8&255),i(c&255)]}function ee(n,c){let i=n.replace(/^#version 300 es\s*/m,"");return c==="vert"?i=i.replace(/\bin\s+(vec\d|float)\s/g,"attribute $1 "):(i=i.replace(/^out\s+vec4\s+fragColor;\s*$/m,""),i=i.replace(/\bfragColor\b/g,"gl_FragColor")),i}function te(n,c,i,h){const t=n.createShader(c);if(!t)throw new Error(`could not create ${h} shader`);if(n.shaderSource(t,i),n.compileShader(t),!n.getShaderParameter(t,n.COMPILE_STATUS)){const e=n.getShaderInfoLog(t)??"(no log)";throw n.deleteShader(t),new Error(`${h} shader failed to compile:
${e}`)}return t}function ue(n,c,i){const h={alpha:!1,antialias:!1,depth:!1,stencil:!1,powerPreference:"high-performance",preserveDrawingBuffer:!1};let t="webgl2",e=i.forceGL1?null:n.getContext("webgl2",h);if(!e){const a=n.getContext("webgl",h);if(!a)return null;e=a,t="webgl1"}const u=window.matchMedia("(pointer: coarse)").matches||window.innerWidth<700?he:de,g=`#define MAX_STEPS ${u.maxSteps}
#define BALLS ${u.balls}
#define AO_TAPS ${u.aoTaps}
`,b=a=>a.includes("#version")?a.replace(/(#version[^\n]*\n)/,`$1${g}`):g+a;let A=ce,f=b(Q);t==="webgl1"&&(A=ee(A,"vert"),f=g+ee(Q,"frag"));let m;try{const a=te(e,e.VERTEX_SHADER,A,"vertex"),o=te(e,e.FRAGMENT_SHADER,f,"fragment"),l=e.createProgram();if(!l)throw new Error("could not create program");if(e.attachShader(l,a),e.attachShader(l,o),e.bindAttribLocation(l,0,"aPos"),e.linkProgram(l),!e.getProgramParameter(l,e.LINK_STATUS))throw new Error(`program failed to link:
${e.getProgramInfoLog(l)??"(no log)"}`);e.deleteShader(a),e.deleteShader(o),m=l}catch(a){return console.error("[gl]",a),null}const y=e.createBuffer();e.bindBuffer(e.ARRAY_BUFFER,y),e.bufferData(e.ARRAY_BUFFER,new Float32Array([-1,-1,3,-1,-1,3]),e.STATIC_DRAW),e.enableVertexAttribArray(0),e.vertexAttribPointer(0,2,e.FLOAT,!1,0,0),e.useProgram(m);const L={resolution:e.getUniformLocation(m,"uResolution"),time:e.getUniformLocation(m,"uTime"),pointer:e.getUniformLocation(m,"uPointer"),pointerActive:e.getUniformLocation(m,"uPointerActive"),press:e.getUniformLocation(m,"uPress"),event:e.getUniformLocation(m,"uEvent"),focus:e.getUniformLocation(m,"uFocus"),paper:e.getUniformLocation(m,"uPaper"),balls:e.getUniformLocation(m,"uBalls")},x=fe(i.paper);e.uniform3f(L.paper,x[0],x[1],x[2]);let w=N,d=0,r=0;function s(){const a=Math.min(window.devicePixelRatio||1,u.maxDpr),o=Math.max(1,Math.round(n.clientWidth*a*w)),l=Math.max(1,Math.round(n.clientHeight*a*w));o===d&&l===r||(d=o,r=l,n.width=o,n.height=l,e.viewport(0,0,o,l),e.uniform2f(L.resolution,o,l))}let p=16,k=0;function ie(a){p+=(a-p)*.08,k++,!(k<45)&&(p>20&&w>Z?(w=Math.max(Z,w-.12),k=0,s()):p<12&&w<N&&(w=Math.min(N,w+.08),k=0,s()))}let D=0,I=!1,H=0,B=0;const P=new Float32Array(u.balls*4);function re(a){for(let o=0;o<u.balls;o++){const l=o*2.3999632;let R=Math.sin(a*(.53+.11*o)+l)+1e-4,C=Math.cos(a*(.47+.09*o)+l*1.7)+1e-4,_=Math.sin(a*(.41+.13*o)+l*2.3)+1e-4;const q=Math.hypot(R,C,_),j=(.45+.25*(o*.6180339%1))*(.88+.12*Math.sin(a*.6+l))/q;P[o*4]=R*j,P[o*4+1]=C*j,P[o*4+2]=_*j,P[o*4+3]=.26+.12*(o*.381966%1)}e.uniform4fv(L.balls,P)}function O(a){const o=c();re(a),e.uniform1f(L.time,a),e.uniform2f(L.pointer,o.pointer[0],o.pointer[1]),e.uniform1f(L.pointerActive,o.pointerActive),e.uniform1f(L.press,o.press),e.uniform1f(L.event,o.eventTime),e.uniform1f(L.focus,o.focus),e.drawArrays(e.TRIANGLES,0,3)}function $(a){if(!I)return;const o=B===0?16:a-B,l=Math.min(o,50);B=a,H+=l/1e3,s(),o<100&&ie(l),O(H),D=requestAnimationFrame($)}function G(a,o){d=a,r=o,n.width=a,n.height=o,e.viewport(0,0,a,o),e.uniform2f(L.resolution,a,o)}function Y(){s(),O(J)}const W=()=>{I?s():Y()};window.addEventListener("resize",W);const K=()=>{document.hidden?z():i.reducedMotion||V()};document.addEventListener("visibilitychange",K);const X=a=>{a.preventDefault(),I=!1,cancelAnimationFrame(D),n.dispatchEvent(new CustomEvent("gl:lost",{bubbles:!0}))};n.addEventListener("webglcontextlost",X);function V(){if(!I){if(i.reducedMotion){Y();return}I=!0,B=0,p=16,k=0,D=requestAnimationFrame($)}}function z(){I=!1,cancelAnimationFrame(D)}return{mode:t,start:V,stop:z,capture(a={}){const{w:o,h:l,type:R="image/png",quality:C}=a;o&&l?G(o,l):s(),O(J);const _=n.toDataURL(R,C);return o&&l&&(d=0),_},bench(a=60,o,l){o&&l?G(o,l):s();const R=new Uint8Array(4),C=()=>e.readPixels(0,0,1,1,e.RGBA,e.UNSIGNED_BYTE,R);O(0),C();const _=performance.now();for(let F=0;F<a;F++)O(F*.016);C();const q=+((performance.now()-_)/a).toFixed(2);return d=0,{msPerFrame:q,buffer:`${o??n.width}x${l??n.height}`}},dispose(){z(),window.removeEventListener("resize",W),document.removeEventListener("visibilitychange",K),n.removeEventListener("webglcontextlost",X),e.deleteProgram(m),e.deleteBuffer(y)}}}const ne=78,oe=11.5,me=3.2,pe=9;function ge(n){let c=0,i=0,h=0,t=0,e=0,v=0,u=0,g=0,b=0,A=0,f=0;function m(r,s){const p=n.getBoundingClientRect();p.width===0||p.height===0||(c=((r-p.left)/p.width*2-1)*(p.width/p.height),i=-((s-p.top)/p.height*2-1))}const y=r=>{m(r.clientX,r.clientY),u=1},L=()=>{u=1},x=()=>{u=0,b=0},w=r=>{m(r.clientX,r.clientY),u=1,b=1},d=()=>{b=0};return window.addEventListener("pointermove",y,{passive:!0}),window.addEventListener("pointerdown",w,{passive:!0}),window.addEventListener("pointerup",d,{passive:!0}),window.addEventListener("pointercancel",x,{passive:!0}),document.addEventListener("pointerenter",L,{passive:!0}),document.addEventListener("pointerleave",x,{passive:!0}),{get position(){return[h,t]},get active(){return g},get press(){return A},sample(){const r=performance.now(),s=f===0?1/60:Math.min((r-f)/1e3,1/30);f=r,e+=((c-h)*ne-e*oe)*s,v+=((i-t)*ne-v*oe)*s,h+=e*s,t+=v*s,g+=(u-g)*Math.min(1,me*s),A+=(b-A)*Math.min(1,pe*s)},dispose(){window.removeEventListener("pointermove",y),window.removeEventListener("pointerdown",w),window.removeEventListener("pointerup",d),window.removeEventListener("pointercancel",x),document.removeEventListener("pointerenter",L),document.removeEventListener("pointerleave",x)}}}const ve=/^[^\s@]+@[^\s@]+\.[^\s@]{2,}$/;function E(n){const c=document.getElementById(n);if(!c)throw new Error(`missing element #${n}`);return c}function we({onSuccess:n,onToggle:c,announce:i}){const h=document.querySelector(".page"),t=E("js-disclose"),e=E("js-panel"),v=E("js-form"),u=E("js-submit"),g=E("js-note"),b=[{input:E("f-name"),err:E("f-name-err"),label:"name"},{input:E("f-email"),err:E("f-email-err"),label:"email"},{input:E("f-message"),err:E("f-message-err"),label:"message"}],A=E("f-gotcha");let f=!1;const m=(d,r)=>{f=d,t.setAttribute("aria-expanded",String(f)),e.classList.toggle("is-open",f),h.classList.toggle("is-expanded",f),document.body.classList.toggle("form-open",f),e.inert=!f,t.querySelector(".disclose-text").textContent=f?"or just email me":"or send a message",t.setAttribute("aria-label",f?"Hide the contact form":"Show the contact form"),c(f),f&&r&&requestAnimationFrame(()=>b[0].input.focus({preventScroll:!0}))};m(!1,!1),t.addEventListener("click",()=>m(!f,!0));function y(d,r){const s=b[d];s.input.closest(".field").classList.toggle("is-invalid",r!==null),s.input.setAttribute("aria-invalid",String(r!==null)),s.err.textContent=r??"",s.err.hidden=r===null,r?s.input.setAttribute("aria-describedby",s.err.id):s.input.removeAttribute("aria-describedby")}function L(){let d=!0;const[r,s,p]=[b[0].input.value.trim(),b[1].input.value.trim(),b[2].input.value.trim()];return r.length<1?(y(0,"Please add your name."),d=!1):y(0,null),s.length<1?(y(1,"Please add an email address."),d=!1):ve.test(s)?y(1,null):(y(1,"That doesn't look like an email address."),d=!1),p.length<2?(y(2,"Please add a message."),d=!1):y(2,null),d}b.forEach((d,r)=>{d.input.addEventListener("input",()=>{d.input.closest(".field").classList.contains("is-invalid")&&y(r,null)})});let x=!1;function w(d,r){g.textContent=d,g.classList.toggle("is-error",r),d&&i(d)}v.addEventListener("submit",async d=>{if(d.preventDefault(),!x){if(A.value.trim()!==""){v.reset(),w("Thanks — message sent.",!1);return}if(!L()){const r=b.filter(s=>s.input.closest(".field").classList.contains("is-invalid"));r[0]?.input.focus(),i(r.length===1?"One field needs attention.":`${r.length} fields need attention.`),g.textContent="",g.classList.remove("is-error");return}x=!0,u.disabled=!0,u.setAttribute("aria-busy","true"),w("Sending…",!1);try{const r=await fetch(se,{method:"POST",headers:{Accept:"application/json"},body:new FormData(v)});if(r.ok)v.reset(),b.forEach((s,p)=>y(p,null)),w("Thanks — message sent. I’ll come back to you shortly.",!1),n();else{const p=(await r.json().catch(()=>null))?.errors?.[0]?.message;w(p??"Something went wrong sending that. Email me directly instead?",!0)}}catch{w("Couldn’t reach the server. Email me directly instead?",!0)}finally{x=!1,u.disabled=!1,u.removeAttribute("aria-busy")}}})}function be(n,c){const i=document.getElementById("js-copy"),h=document.getElementById("js-copy-label");if(!i||!h)return;if(!navigator.clipboard){i.remove();return}i.setAttribute("aria-label",`Copy ${n} to clipboard`);let t=0;i.addEventListener("click",async()=>{try{await navigator.clipboard.writeText(n),h.textContent="Copied",i.classList.add("is-done"),c("Email address copied to clipboard"),clearTimeout(t),t=window.setTimeout(()=>{h.textContent="Copy",i.classList.remove("is-done")},2e3)}catch{h.textContent="Press ⌘C",clearTimeout(t),t=window.setTimeout(()=>{h.textContent="Copy"},2500)}})}const M=document.getElementById("stage"),S=document.getElementById("poster"),ye=document.getElementById("js-status"),ae=n=>{ye.textContent=n};function Le(){document.getElementById("js-name").textContent=T.name,document.getElementById("js-role").textContent=T.role;const n=document.getElementById("js-email");n.textContent=T.email,n.href=`mailto:${T.email}`;const c=document.getElementById("js-availability");c.textContent=T.availability,document.getElementById("js-github").remove(),document.title=`${T.name} — ${T.role}`}function U(){M.hidden=!0,S.hidden=!1,S.addEventListener("load",()=>S.classList.add("is-shown"),{once:!0}),S.addEventListener("error",()=>{S.hidden=!0},{once:!0}),S.getAttribute("src")?S.complete&&S.naturalWidth>0&&S.classList.add("is-shown"):S.src=S.dataset.src??""}function Ae(){Le();const n=new URLSearchParams(location.search),c=n.has("static"),i=window.matchMedia("(prefers-reduced-motion: reduce)").matches||n.has("reduced");let h=-1;const t=()=>{h=performance.now()};let e=0,v=0,u=0;if(be(T.email,ae),we({onSuccess:t,onToggle:f=>{e=f?1:0},announce:ae}),c){U();return}const g=ge(M),A=ue(M,()=>{g.sample();const f=performance.now(),m=u===0?1/60:Math.min((f-u)/1e3,1/30);return u=f,v+=(e-v)*Math.min(1,4.5*m),{pointer:g.position,pointerActive:g.active,press:g.press,eventTime:h<0?999:(f-h)/1e3,focus:v}},{paper:le.paper,reducedMotion:i,forceGL1:n.has("gl1")});if(!A){g.dispose(),U();return}M.addEventListener("gl:lost",()=>{console.warn("[gl] context lost — falling back to the still poster"),U()}),A.start()}Ae();
