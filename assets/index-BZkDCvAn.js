(function(){const r=document.createElement("link").relList;if(r&&r.supports&&r.supports("modulepreload"))return;for(const n of document.querySelectorAll('link[rel="modulepreload"]'))l(n);new MutationObserver(n=>{for(const e of n)if(e.type==="childList")for(const w of e.addedNodes)w.tagName==="LINK"&&w.rel==="modulepreload"&&l(w)}).observe(document,{childList:!0,subtree:!0});function a(n){const e={};return n.integrity&&(e.integrity=n.integrity),n.referrerPolicy&&(e.referrerPolicy=n.referrerPolicy),n.crossOrigin==="use-credentials"?e.credentials="include":n.crossOrigin==="anonymous"?e.credentials="omit":e.credentials="same-origin",e}function l(n){if(n.ep)return;n.ep=!0;const e=a(n);fetch(n.href,e)}})();const P={name:"Stephen Cronk",role:"Web developer",email:"stephenc.dev@gmail.com",availability:"Available for new work"},ue="https://formspree.io/f/xlgqajrg",fe={paper:"#f4f3ef"},me=`#version 300 es
...
`,F=7,ve=[0,-.49,0,.29,0,.49,0,.29,.85,-.49,0,0,.29,.49,0,0,.29,.85,0,.1,0,.16,0,.35,0,.1,.5,-.15,-.3,0,.09,-.05,-.1,0,.09,.3,.15,-.3,0,.09,.05,-.1,0,.09,.3,0,-.3,-.15,.09,0,-.1,-.05,.09,.3,0,-.35,0,.14,0,-.5,0,.1,.4],ge=[0,-.62,0,.215,0,-.25,0,.365,0,0,-.25,0,.365,0,.1,0,.404,0,0,.1,0,.404,0,.78,0,.025,0,-.5,-.78,0,.05,-.1,-.36,0,.13,.8,.5,-.78,0,.05,.1,-.36,0,.13,.8,0,-.78,-.5,.05,0,-.36,-.1,.13,.8,0,-.62,0,.2,0,-.8,0,.11,0],_=[{shape:null,hold:4.5,morph:1.5},{shape:ve,hold:3,morph:1.4},{shape:ge,hold:3,morph:1.5}],be=_.reduce((t,r)=>t+r.hold+r.morph,0),we=.004,ye=.03,te=.012,ne=.05,xe=.2,Se=9,Ae=t=>t*t*(3-2*t),M=(t,r,a)=>t+(r-t)*a;function Le(){const t=new Float32Array(F*4),r=new Float32Array(F*4),a=new Float32Array(F),l={partA:t,partB:r,partSq:a,mix:0,k:ne,wobble:te,spin:0};function n(e,w,u){for(let f=0;f<F;f++){const h=f*Se;for(let m=0;m<4;m++)t[f*4+m]=M(e[h+m],w[h+m],u),r[f*4+m]=M(e[h+4+m],w[h+4+m],u);a[f]=M(e[h+8],w[h+8],u)}}return function(w){let u=w%be,f=0;for(let g=0;g<_.length;g++){const S=_[g];if(u<S.hold+S.morph){f=g;break}u-=S.hold+S.morph}const h=_[f],m=_[(f+1)%_.length],v=u>h.hold,d=v?Ae((u-h.hold)/h.morph):0;h.shape&&m.shape?(n(h.shape,m.shape,d),l.mix=1):h.shape?(n(h.shape,h.shape,0),l.mix=1-d):m.shape?(n(m.shape,m.shape,0),l.mix=d):l.mix=0;const x=v?Math.sin(d*Math.PI):0;return l.k=M(ne,xe,x),l.wobble=M(h.shape||v&&m.shape?we:te,ye,x),l.spin=Math.sin(w*.32)*.55,l}}const Ee={maxSteps:128,balls:7,aoTaps:5,maxDpr:2},Pe={maxSteps:72,balls:5,aoTaps:3,maxDpr:1.5},oe=.55,$=1,ae=2.4;function Te(t){const r=parseInt(t.replace("#",""),16),a=l=>{const n=l/255;return n<=.04045?n/12.92:Math.pow((n+.055)/1.055,2.4)};return[a(r>>16&255),a(r>>8&255),a(r&255)]}function ie(t,r){let a=t.replace(/^#version 300 es\s*/m,"");return r==="vert"?a=a.replace(/\bin\s+(vec\d|float)\s/g,"attribute $1 "):(a=a.replace(/^out\s+vec4\s+fragColor;\s*$/m,""),a=a.replace(/\bfragColor\b/g,"gl_FragColor")),a}function se(t,r,a,l){const n=t.createShader(r);if(!n)throw new Error(`could not create ${l} shader`);if(t.shaderSource(n,a),t.compileShader(n),!t.getShaderParameter(n,t.COMPILE_STATUS)){const e=t.getShaderInfoLog(n)??"(no log)";throw t.deleteShader(n),new Error(`${l} shader failed to compile:
${e}`)}return n}function Re(t,r,a){const l={alpha:!1,antialias:!1,depth:!1,stencil:!1,powerPreference:"high-performance",preserveDrawingBuffer:!1};let n="webgl2",e=a.forceGL1?null:t.getContext("webgl2",l);if(!e){const i=t.getContext("webgl",l);if(!i)return null;e=i,n="webgl1"}const u=window.matchMedia("(pointer: coarse)").matches||window.innerWidth<700?Pe:Ee,f=`#define MAX_STEPS ${u.maxSteps}
#define BALLS ${u.balls}
#define PARTS ${F}
#define AO_TAPS ${u.aoTaps}
`,h=i=>i.includes("#version")?i.replace(/(#version[^\n]*\n)/,`$1${f}`):f+i;let m=me,v=h(ee);n==="webgl1"&&(m=ie(m,"vert"),v=f+ie(ee,"frag"));let d;try{const i=se(e,e.VERTEX_SHADER,m,"vertex"),o=se(e,e.FRAGMENT_SHADER,v,"fragment"),s=e.createProgram();if(!s)throw new Error("could not create program");if(e.attachShader(s,i),e.attachShader(s,o),e.bindAttribLocation(s,0,"aPos"),e.linkProgram(s),!e.getProgramParameter(s,e.LINK_STATUS))throw new Error(`program failed to link:
${e.getProgramInfoLog(s)??"(no log)"}`);e.deleteShader(i),e.deleteShader(o),d=s}catch(i){return console.error("[gl]",i),null}const x=e.createBuffer();e.bindBuffer(e.ARRAY_BUFFER,x),e.bufferData(e.ARRAY_BUFFER,new Float32Array([-1,-1,3,-1,-1,3]),e.STATIC_DRAW),e.enableVertexAttribArray(0),e.vertexAttribPointer(0,2,e.FLOAT,!1,0,0),e.useProgram(d);const g={resolution:e.getUniformLocation(d,"uResolution"),time:e.getUniformLocation(d,"uTime"),pointer:e.getUniformLocation(d,"uPointer"),pointerActive:e.getUniformLocation(d,"uPointerActive"),press:e.getUniformLocation(d,"uPress"),event:e.getUniformLocation(d,"uEvent"),focus:e.getUniformLocation(d,"uFocus"),paper:e.getUniformLocation(d,"uPaper"),balls:e.getUniformLocation(d,"uBalls"),partA:e.getUniformLocation(d,"uPartA"),partB:e.getUniformLocation(d,"uPartB"),partSq:e.getUniformLocation(d,"uPartSq"),shapeMix:e.getUniformLocation(d,"uShapeMix"),shapeK:e.getUniformLocation(d,"uShapeK"),shapeSpin:e.getUniformLocation(d,"uShapeSpin"),wobble:e.getUniformLocation(d,"uWobble")},S=Te(a.paper);e.uniform3f(g.paper,S[0],S[1],S[2]);let A=$,b=0,c=0;function p(){const i=Math.min(window.devicePixelRatio||1,u.maxDpr),o=Math.max(1,Math.round(t.clientWidth*i*A)),s=Math.max(1,Math.round(t.clientHeight*i*A));o===b&&s===c||(b=o,c=s,t.width=o,t.height=s,e.viewport(0,0,o,s),e.uniform2f(g.resolution,o,s))}let y=16,k=0;function he(i){y+=(i-y)*.08,k++,!(k<45)&&(y>20&&A>oe?(A=Math.max(oe,A-.12),k=0,p()):y<12&&A<$&&(A=Math.min($,A+.08),k=0,p()))}let z=0,R=!1,W=0,q=0;const B=new Float32Array(u.balls*4);function de(i){for(let o=0;o<u.balls;o++){const s=o*2.3999632;let C=Math.sin(i*(.53+.11*o)+s)+1e-4,T=Math.cos(i*(.47+.09*o)+s*1.7)+1e-4,I=Math.sin(i*(.41+.13*o)+s*2.3)+1e-4;const D=Math.hypot(C,T,I),H=(.45+.25*(o*.6180339%1))*(.88+.12*Math.sin(i*.6+s))/D;B[o*4]=C*H,B[o*4+1]=T*H,B[o*4+2]=I*H,B[o*4+3]=.26+.12*(o*.381966%1)}e.uniform4fv(g.balls,B)}const pe=Le();function O(i){const o=r();de(i);const s=pe(i);e.uniform4fv(g.partA,s.partA),e.uniform4fv(g.partB,s.partB),e.uniform1fv(g.partSq,s.partSq),e.uniform1f(g.shapeMix,s.mix),e.uniform1f(g.shapeK,s.k),e.uniform1f(g.shapeSpin,s.spin),e.uniform1f(g.wobble,s.wobble),e.uniform1f(g.time,i),e.uniform2f(g.pointer,o.pointer[0],o.pointer[1]),e.uniform1f(g.pointerActive,o.pointerActive),e.uniform1f(g.press,o.press),e.uniform1f(g.event,o.eventTime),e.uniform1f(g.focus,o.focus),e.drawArrays(e.TRIANGLES,0,3)}function Y(i){if(!R)return;const o=q===0?16:i-q,s=Math.min(o,50);q=i,W+=s/1e3,p(),o<100&&he(s),O(W),z=requestAnimationFrame(Y)}function G(i,o){b=i,c=o,t.width=i,t.height=o,e.viewport(0,0,i,o),e.uniform2f(g.resolution,i,o)}function X(){p(),O(ae)}const V=()=>{R?p():X()};window.addEventListener("resize",V);const Q=()=>{document.hidden?N():a.reducedMotion||J()};document.addEventListener("visibilitychange",Q);const Z=i=>{i.preventDefault(),R=!1,cancelAnimationFrame(z),t.dispatchEvent(new CustomEvent("gl:lost",{bubbles:!0}))};t.addEventListener("webglcontextlost",Z);function J(){if(!R){if(a.reducedMotion){X();return}R=!0,q=0,y=16,k=0,z=requestAnimationFrame(Y)}}function N(){R=!1,cancelAnimationFrame(z)}return{mode:n,start:J,stop:N,capture(i={}){const{w:o,h:s,type:C="image/png",quality:T,t:I=ae}=i;o&&s?G(o,s):p(),O(I);const D=t.toDataURL(C,T);return o&&s&&(b=0),D},bench(i=60,o,s){o&&s?G(o,s):p();const C=new Uint8Array(4),T=()=>e.readPixels(0,0,1,1,e.RGBA,e.UNSIGNED_BYTE,C);O(0),T();const I=performance.now();for(let U=0;U<i;U++)O(U*.016);T();const D=+((performance.now()-I)/i).toFixed(2);return b=0,{msPerFrame:D,buffer:`${o??t.width}x${s??t.height}`}},dispose(){N(),window.removeEventListener("resize",V),document.removeEventListener("visibilitychange",Q),t.removeEventListener("webglcontextlost",Z),e.deleteProgram(d),e.deleteBuffer(x)}}}const re=78,le=11.5,Ce=3.2,Ie=9;function _e(t){let r=0,a=0,l=0,n=0,e=0,w=0,u=0,f=0,h=0,m=0,v=0;function d(c,p){const y=t.getBoundingClientRect();y.width===0||y.height===0||(r=((c-y.left)/y.width*2-1)*(y.width/y.height),a=-((p-y.top)/y.height*2-1))}const x=c=>{d(c.clientX,c.clientY),u=1},g=()=>{u=1},S=()=>{u=0,h=0},A=c=>{d(c.clientX,c.clientY),u=1,h=1},b=()=>{h=0};return window.addEventListener("pointermove",x,{passive:!0}),window.addEventListener("pointerdown",A,{passive:!0}),window.addEventListener("pointerup",b,{passive:!0}),window.addEventListener("pointercancel",S,{passive:!0}),document.addEventListener("pointerenter",g,{passive:!0}),document.addEventListener("pointerleave",S,{passive:!0}),{get position(){return[l,n]},get active(){return f},get press(){return m},sample(){const c=performance.now(),p=v===0?1/60:Math.min((c-v)/1e3,1/30);v=c,e+=((r-l)*re-e*le)*p,w+=((a-n)*re-w*le)*p,l+=e*p,n+=w*p,f+=(u-f)*Math.min(1,Ce*p),m+=(h-m)*Math.min(1,Ie*p)},dispose(){window.removeEventListener("pointermove",x),window.removeEventListener("pointerdown",A),window.removeEventListener("pointerup",b),window.removeEventListener("pointercancel",S),document.removeEventListener("pointerenter",g),document.removeEventListener("pointerleave",S)}}}const ke=/^[^\s@]+@[^\s@]+\.[^\s@]{2,}$/;function L(t){const r=document.getElementById(t);if(!r)throw new Error(`missing element #${t}`);return r}function Be({onSuccess:t,onToggle:r,announce:a}){const l=document.querySelector(".page"),n=L("js-disclose"),e=L("js-panel"),w=L("js-form"),u=L("js-submit"),f=L("js-note"),h=[{input:L("f-name"),err:L("f-name-err"),label:"name"},{input:L("f-email"),err:L("f-email-err"),label:"email"},{input:L("f-message"),err:L("f-message-err"),label:"message"}],m=L("f-gotcha");let v=!1;const d=(b,c)=>{v=b,n.setAttribute("aria-expanded",String(v)),e.classList.toggle("is-open",v),l.classList.toggle("is-expanded",v),document.body.classList.toggle("form-open",v),e.inert=!v,n.querySelector(".disclose-text").textContent=v?"or just email me":"or send a message",n.setAttribute("aria-label",v?"Hide the contact form":"Show the contact form"),r(v),v&&c&&requestAnimationFrame(()=>h[0].input.focus({preventScroll:!0}))};d(!1,!1),n.addEventListener("click",()=>d(!v,!0));function x(b,c){const p=h[b];p.input.closest(".field").classList.toggle("is-invalid",c!==null),p.input.setAttribute("aria-invalid",String(c!==null)),p.err.textContent=c??"",p.err.hidden=c===null,c?p.input.setAttribute("aria-describedby",p.err.id):p.input.removeAttribute("aria-describedby")}function g(){let b=!0;const[c,p,y]=[h[0].input.value.trim(),h[1].input.value.trim(),h[2].input.value.trim()];return c.length<1?(x(0,"Please add your name."),b=!1):x(0,null),p.length<1?(x(1,"Please add an email address."),b=!1):ke.test(p)?x(1,null):(x(1,"That doesn't look like an email address."),b=!1),y.length<2?(x(2,"Please add a message."),b=!1):x(2,null),b}h.forEach((b,c)=>{b.input.addEventListener("input",()=>{b.input.closest(".field").classList.contains("is-invalid")&&x(c,null)})});let S=!1;function A(b,c){f.textContent=b,f.classList.toggle("is-error",c),b&&a(b)}w.addEventListener("submit",async b=>{if(b.preventDefault(),!S){if(m.value.trim()!==""){w.reset(),A("Thanks — message sent.",!1);return}if(!g()){const c=h.filter(p=>p.input.closest(".field").classList.contains("is-invalid"));c[0]?.input.focus(),a(c.length===1?"One field needs attention.":`${c.length} fields need attention.`),f.textContent="",f.classList.remove("is-error");return}S=!0,u.disabled=!0,u.setAttribute("aria-busy","true"),A("Sending…",!1);try{const c=await fetch(ue,{method:"POST",headers:{Accept:"application/json"},body:new FormData(w)});if(c.ok)w.reset(),h.forEach((p,y)=>x(y,null)),A("Thanks — message sent. I’ll come back to you shortly.",!1),t();else{const y=(await c.json().catch(()=>null))?.errors?.[0]?.message;A(y??"Something went wrong sending that. Email me directly instead?",!0)}}catch{A("Couldn’t reach the server. Email me directly instead?",!0)}finally{S=!1,u.disabled=!1,u.removeAttribute("aria-busy")}}})}function Oe(t,r){const a=document.getElementById("js-copy"),l=document.getElementById("js-copy-label");if(!a||!l)return;if(!navigator.clipboard){a.remove();return}a.setAttribute("aria-label",`Copy ${t} to clipboard`);let n=0;a.addEventListener("click",async()=>{try{await navigator.clipboard.writeText(t),l.textContent="Copied",a.classList.add("is-done"),r("Email address copied to clipboard"),clearTimeout(n),n=window.setTimeout(()=>{l.textContent="Copy",a.classList.remove("is-done")},2e3)}catch{l.textContent="Press ⌘C",clearTimeout(n),n=window.setTimeout(()=>{l.textContent="Copy"},2500)}})}const j=document.getElementById("stage"),E=document.getElementById("poster"),De=document.getElementById("js-status"),ce=t=>{De.textContent=t};function Me(){document.getElementById("js-name").textContent=P.name,document.getElementById("js-role").textContent=P.role;const t=document.getElementById("js-email");t.textContent=P.email,t.href=`mailto:${P.email}`;const r=document.getElementById("js-availability");r.textContent=P.availability,document.getElementById("js-github").remove(),document.title=`${P.name} — ${P.role}`}function K(){j.hidden=!0,E.hidden=!1,E.addEventListener("load",()=>E.classList.add("is-shown"),{once:!0}),E.addEventListener("error",()=>{E.hidden=!0},{once:!0}),E.getAttribute("src")?E.complete&&E.naturalWidth>0&&E.classList.add("is-shown"):E.src=E.dataset.src??""}function Fe(){Me();const t=new URLSearchParams(location.search),r=t.has("static"),a=window.matchMedia("(prefers-reduced-motion: reduce)").matches||t.has("reduced");let l=-1;const n=()=>{l=performance.now()};let e=0,w=0,u=0;if(Oe(P.email,ce),Be({onSuccess:n,onToggle:v=>{e=v?1:0},announce:ce}),r){K();return}const f=_e(j),m=Re(j,()=>{f.sample();const v=performance.now(),d=u===0?1/60:Math.min((v-u)/1e3,1/30);return u=v,w+=(e-w)*Math.min(1,4.5*d),{pointer:f.position,pointerActive:f.active,press:f.press,eventTime:l<0?999:(v-l)/1e3,focus:w}},{paper:fe.paper,reducedMotion:a,forceGL1:t.has("gl1")});if(!m){f.dispose(),K();return}j.addEventListener("gl:lost",()=>{console.warn("[gl] context lost — falling back to the still poster"),K()}),m.start()}Fe();
